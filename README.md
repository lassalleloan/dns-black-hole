...
3. Running `post-run` script
   1. Creation of hard link between /etc/hosts and `$WORKING_DIRECTORY`/dns-black-hole/src/etc/hosts.hardlink (variable in the configuration file .conf)
...
The only file affected by scripts is /etc/hosts. They create a hard link between /etc/hosts and `$WORKING_DIRECTORY`/dns-black-hole/src/etc/hosts.hardlink variable in the configuration file .conf.  
